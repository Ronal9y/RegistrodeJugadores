...
class CreateMovimientoLocalUseCase @Inject constructor(
    private val repository: MovimientosRepository
) {
    suspend operator fun invoke(movimiento: Movimiento): Resource<Movimiento> =
        repository.createMovimientoLocal(movimiento)
}